...
@app.route("/")
def str1():
    return '<h1>Хочешь увидеть интересные факты или подбросить моентку? Нажми ниже!</h1>''<a href="/random_fact">Посмотреть случайный факт!</a>''<a href="/monetka">Подбросить монетку!</a>'
    
@app.route("/random_fact")
def str2():
    return f'<h1>{random.choice(fact)}</h1>''<a href="/random_fact">Другой</a>'

@app.route("/monetka")
def str3():
    return f'<h1>{random.choice(orel_or_reshka)}</h1>''<a href="/monetka">Подпросить ещё раз</a>'
